...
func NewDialectPostgres() *DialectPostgres {
	return &DialectPostgres{}
}

func (p *DialectPostgres) Init(schema *Schema) error {

	schema.insertSql["block"] =
		fmt.Sprintf("INSERT INTO %s (number, hash, timestamp) VALUES ($1, $2, $3) RETURNING number", TableName(schema, "block"))
	...
	schema.insertSql["cursor"] =
		fmt.Sprintf("INSERT INTO %s (name, cursor) VALUES ($1, $2) ON CONFLICT (name) DO UPDATE SET cursor = $2", TableName(schema, "cursor"))
	return nil
}

func (p *DialectPostgres) HandleTable(schema *Schema, table *Table) error {
	err := p.createTable(schema, table)
	if err != nil {
		return fmt.Errorf("creating table %q: %w", table.Name, err)
	}

	err = createInsertFromDescriptor(schema, table)
	if err != nil {
...
func (p *DialectPostgres) createTable(schema *Schema, table *Table) error {
...
func createInsertFromDescriptor(schema *Schema, table *Table) error {
	tableName := table.FullName(schema)
